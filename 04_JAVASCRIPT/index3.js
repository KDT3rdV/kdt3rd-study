...
// return 키워드 : 반환값-> 함수내부 코드의 '최종 결과값'
...
countNum(); // 함수 호출
// console.log('count 값2: ', count); //Uncaught ReferenceError: count is not defined 에러 
// 지역변수 
// -> 함수 안에서 선언한 변수는 함수 안에서만 유효하다
// -> 지역변수는 함수의 body를 벗어나면 접근할 수 없다.

let count2 = 0; //함수 밖에서 변수 선언 - 전역변수
console.log('count2 값1: ', count2); // 0
function countNum2() {
    count2 = 77; // 함수 안에서 count2 재할당
...
printColor();
console.log(color); // red
// 지역변수와 전역변수의 이름이 동일한 경우 함수 안에서는 지역변수 사용
...
//================================================================
// 문자열 속성과 메서드(함수)
// : 자바스크립트에서 기본제공하는 문자열(string)에서 사용가능한 편리한 기능
...
console.log(song.length); // song변수의 문자열 길이(공백포함)
console.log(song2.length); // 특수문자도 포함
...
// 인덱싱 (indexing) : string에서 한글자를 선택 가능
// 변수명[위치값]
// 위치값: 숫자 "0"부터 시작 
console.log(song); //Strawberry Moon

console.log(song[0]); // 첫글자: 컴퓨터는 항상 0부터 숫자 시작
console.log(song[1]);
console.log(song[2]);
console.log(typeof(song[0])); // string 자료형
