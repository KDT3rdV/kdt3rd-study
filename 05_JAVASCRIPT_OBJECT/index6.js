...
todoForm.addEventListener('submit', function(event) {
    // form의 submit 이벤트는 제출되는것이 기본 동작
    // 제출하면 웹이 새로고침 됨
    // console.log('Add 버튼 클릭');

    // preventDefault(): 브라우저에서 구현된 기본동작 취소
    ...
    // li태그를 만들어서, li태그의 content로 input.value 값이 담기도록
...
chgInput.addEventListener('input', function () {
    ...
    const div1 = document.querySelector('.intro1');
    div1.textContent = chgInput.value;
...
const chgText = document.querySelector('#change-text');
console.log(chgText);

chgText.addEventListener('input', function () {
    // input: 입력창에 변경이 일어나는 순간을 계속 감지

    const div2 = document.querySelector('.intro2');
    div2.textContent = chgText.value;
...
keyOutput.addEventListener('keyup', function(event){
    // keyup: 키를 누르고 있다가 뗄 때
    // console.log(event);
    // console.log(event.code);
